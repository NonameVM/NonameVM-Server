...
export class User {
    ...
    TurnRateLimit;
    VoteRateLimit;
    constructor(ws, ip, config, username, node) {
        this.IP = ip;
    ...
    assignGuestName(existingUsers) {
        var username;
        do {
            username = "user" + Utilities.Randint(10000, 99999);
    ...
    onMsgSent() {
        if (!this.Config.collabvm.automute.enabled)
            return;
        if (this.rank !== 0)
            return;
    ...
    mute(permanent) {
        ...
        log("INFO", `Muted ${this.IP.address} : (${this.username})`);
        if (!permanent) {
    ...
    unmute() {
        ...
        log("INFO", `Unmuted ${this.IP.address} : (${this.username})`);
    }
    banCmdArgs(arg) {
    ...
    async ban() {
        ...
        try {
            if (Array.isArray(this.Config.collabvm.bancmd)) {
                let args = this.Config.collabvm.bancmd.map((a) => this.banCmdArgs(a));
                if (args.length || args[0].length) {
                    await execa(args.shift(), args, { stdout: process.stdout, stderr: process.stderr });
                    this.kick();
		    log("INFO", `Sucefully banned ${this.IP.address} : (${this.username})`);
                }
                else {
                ...
                let cmd = this.banCmdArgs(this.Config.collabvm.bancmd);
                if (cmd.length) {
            ...
            log("ERROR", `Failed to ban ${this.IP.address} (${this.username}): ${e.shortMessage}`);
        }
    }
    async kick() {
