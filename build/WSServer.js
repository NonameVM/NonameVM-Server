...
export default class WSServer {
    ...
    constructor(config, vm) {
        ...
        this.server.on('upgrade', (req, socket, head) => this.httpOnUpgrade(req, socket, head));
    ...
    ;
    connectionClosed(user) {
        if (user.IP.vote != null) {
            user.IP.vote = null;
            this.sendVoteUpdate();
        }
        ;
        if (this.indefiniteTurn === user)
            this.indefiniteTurn = null;
        ...
        switch (msgArr[0]) {
            ...
            case "connect":
                ...
                if (this.ChatHistory.size !== 0)
                    client.sendMsg(this.getChatHistoryMsg());
                if (this.Config.collabvm.motd)
                    client.sendMsg(guacutils.encode("chat", "", this.Config.collabvm.motd));
            ...
            case "view":
                ...
                break;
            case "rename":
                if (!client.RenameRateLimit.request())
                    return;
                if (client.connectedToNode && client.IP.muted)
                    return;
            ...
            case "chat":
                ...
                break;
            case "turn":
                if ((!this.turnsAllowed || this.Config.collabvm.turnwhitelist) && client.rank !== Rank.Admin && client.rank !== Rank.Moderator && client.rank !== Rank.Turn)
                    return;
                if (!client.TurnRateLimit.request())
                    return;
                if (!client.connectedToNode)
                    return;
                if (msgArr.length > 2)
                ...
                if (takingTurn) {
                    ...
                    if (currentQueue.indexOf(client) !== -1)
                        return;
                ...
                break;
            case "mouse":
                if (this.TurnQueue.peek() !== client && client.rank !== Rank.Admin)
                    return;
                if (!this.VM.acceptingInput())
                    return;
                var x = parseInt(msgArr[1]);
                var y = parseInt(msgArr[2]);
                var mask = parseInt(msgArr[3]);
                if (x === undefined || y === undefined || mask === undefined)
                    return;
            ...
            case "key":
                ...
                if (keysym === undefined || (down !== 0 && down !== 1))
                    return;
                this.VM.keyEvent(keysym, down === 1 ? true : false);
                break;
            case "vote":
                if (!this.Config.vm.snapshots)
                    return;
                if ((!this.turnsAllowed || this.Config.collabvm.turnwhitelist) && client.rank !== Rank.Admin && client.rank !== Rank.Moderator && client.rank !== Rank.Turn)
                    return;
                if (!client.connectedToNode)
                ...
                if (msgArr.length !== 2)
                    return;
                if (!client.VoteRateLimit.request())
                    return;
                switch (msgArr[1]) {
                    case "1":
                        if (!this.voteInProgress) {
            ...
            case "admin":
                ...
                switch (msgArr[1]) {
                    case "2":
                        // Login
                        if (!client.LoginRateLimit.request() || !client.username)
                            return;
                        if (msgArr.length !== 3)
                            return;
                    ...
                    case "5":
                        // QEMU Monitor
                        if (client.rank !== Rank.Admin && (client.rank !== Rank.Moderator || !this.Config.collabvm.moderatorPermissions.qemumonitor))
                            return;
                        ...
                        if (msgArr.length !== 4 || msgArr[2] !== this.Config.collabvm.node)
                            return;
                    ...
                    case "8":
                        // Restore
                        if (client.rank !== Rank.Admin && (client.rank !== Rank.Moderator || !this.Config.collabvm.moderatorPermissions.restore))
                            return;
                        this.VM.Restore();
                        break;
                    case "10":
                        // Reboot
                        if (client.rank !== Rank.Admin && (client.rank !== Rank.Moderator || !this.Config.collabvm.moderatorPermissions.reboot))
                            return;
                        if (msgArr.length !== 3 || msgArr[2] !== this.Config.collabvm.node)
                            return;
                        this.VM.Reboot();
                        break;
                    case "12":
                        // Ban
                        if (client.rank !== Rank.Admin && (client.rank !== Rank.Moderator || !this.Config.collabvm.moderatorPermissions.ban))
                            return;
                    ...
                    case "13":
                        ...
                        if (msgArr.length !== 3)
                            return;
                        if (client.rank !== Rank.Admin && (client.rank !== Rank.Moderator || !this.Config.collabvm.moderatorPermissions.forcevote))
                            return;
                        if (!this.voteInProgress)
                            return;
                        switch (msgArr[2]) {
                    ...
                    case "14":
                        // Mute
                        if (client.rank !== Rank.Admin && (client.rank !== Rank.Moderator || !this.Config.collabvm.moderatorPermissions.mute))
                            return;
                        if (msgArr.length !== 4)
                            return;
                        var user = this.clients.find(c => c.username === msgArr[2]);
                    ...
                    case "15":
                        // Kick
                        if (client.rank !== Rank.Admin && (client.rank !== Rank.Moderator || !this.Config.collabvm.moderatorPermissions.kick))
                            return;
                        var user = this.clients.find(c => c.username === msgArr[2]);
                        if (!user)
                            return;
                    ...
                    case "16":
                        // End turn
                        if (client.rank !== Rank.Admin && (client.rank !== Rank.Moderator || !this.Config.collabvm.moderatorPermissions.bypassturn))
                            return;
                        if (msgArr.length !== 3)
                            return;
                        var user = this.clients.find(c => c.username === msgArr[2]);
                    ...
                    case "18":
                        // Rename user
                        if (client.rank !== Rank.Admin && (client.rank !== Rank.Moderator || !this.Config.collabvm.moderatorPermissions.rename))
                            return;
                        if (msgArr.length !== 4)
                            return;
                        var user = this.clients.find(c => c.username === msgArr[2]);
                    ...
                    case "19":
                        // Get IP
                        if (client.rank !== Rank.Admin && (client.rank !== Rank.Moderator || !this.Config.collabvm.moderatorPermissions.grabip))
                            return;
                        if (msgArr.length !== 3)
                            return;
                        var user = this.clients.find(c => c.username === msgArr[2]);
                    ...
                    case "21":
                        // XSS
                        if (client.rank !== Rank.Admin && (client.rank !== Rank.Moderator || !this.Config.collabvm.moderatorPermissions.xss))
                            return;
                        if (msgArr.length !== 3)
                            return;
                        switch (client.rank) {
                    ...
                    case "22":
                        // Toggle turns
                        if (client.rank !== Rank.Admin && (client.rank !== Rank.Moderator || !this.Config.collabvm.moderatorPermissions.toggleturns))
                            return;
                        if (msgArr.length !== 3)
                            return;
                        switch (msgArr[2]) {
                    ...
                    case "23":
                        // Indefinite turn
                        if (client.rank !== Rank.Admin && (client.rank !== Rank.Moderator || !this.Config.collabvm.moderatorPermissions.indefeniteturns))
                            return;
                    ...
                    case "24":
                        // Hide screen
                        if (client.rank !== Rank.Admin && (client.rank !== Rank.Moderator || !this.Config.collabvm.moderatorPermissions.hidescreen))
                            return;
                        if (msgArr.length !== 3)
                            return;
                        switch (msgArr[2]) {
                        ...
                        case "25":
                            if (client.rank !== Rank.Admin && (client.rank !== Rank.Moderator || !this.Config.collabvm.moderatorPermissions.servermessage))
                                return;
                            switch (msgArr[2]) {
                                case msgArr[2]:
    ...
    renameUser(client, newName) {
        ...
        var hadName = client.username ? true : false;
        var oldname;
        if (hadName)
    ...
    getAdduserMsg() {
        var arr = ["adduser", this.clients.filter(c => c.username).length.toString()];
    ...
    sendTurnUpdate(client) {
        ...
        this.clients.filter(c => (c !== currentTurningUser && c.connectedToNode)).forEach((c) => {
            if (turnQueueArr.indexOf(c) !== -1) {
                var time;
                if (this.indefiniteTurn === null)
                    time = ((this.TurnTime * 1000) + ((turnQueueArr.indexOf(c) - 1) * this.Config.collabvm.turnTime * 1000));
                else
                    time = 9999999999;
    ...
    endVote(result) {
        ...
        if (result === true || (result === undefined && count.yes >= count.no)) {
            this.clients.forEach(c => c.sendMsg(guacutils.encode("chat", "", "The last snapshot of the virtual machine has been successfully restored.")));
            this.VM.Restore();
        }
        else {
            this.clients.forEach(c => c.sendMsg(guacutils.encode("chat", "", "The snapshot of the virtual machine was not restored because the vote was lost.")));
        }
