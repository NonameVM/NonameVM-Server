...
export default async function BatchRects(fb, rects) {
    ...
    rects.forEach(r => {
        if (((r.height + r.y) - mergedY) > mergedHeight)
            mergedHeight = (r.height + r.y) - mergedY;
        if (((r.width + r.x) - mergedX) > mergedWidth)
            mergedWidth = (r.width + r.x) - mergedX;
    });
